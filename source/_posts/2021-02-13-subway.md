...
距离上一篇《好地段是怎么选出来的-从北京地铁看区域的重要性》已经过去八年有余，一直念叨着重新更新一下北京的地铁数据。话说 deadline 是第一生产力，春节前立了 flag，今天把它拔了。
...
这里面有一些细节，比如地铁网络的边是有权重的。一种做法是将站点和站点之间的权重用**两个站点距离的倒数**来度量，没有经纬度的站点之间的权重统一设置为全部边的平均值。但考虑坐地铁的实际体验，这个距离一般还真不一定是关键因素，所以我粗暴地去掉了边权重这个指标。

有了站点数据后，就可以构建北京地铁的网络拓扑图，进而度量和描述每个地铁站在全网络的作用。可问题又来了，我们应该用什么指标来衡量呢？度量方式依然采用了《好地段是怎么选出来的-从北京地铁看区域的重要性》中的三个指标：
...
3. Closeness centrality：如果从地铁网络中 A 点，到所有其他地铁站的“最短距离”的和最小的话，那这个站点一定是最便捷的，它反映的是网络中节点 A 到其他节点的平均最短距离。
...
> 聚类是一种统计方法。如果我们将样本分为多个类，聚类方法可以保证类和类之间的差异度足够大，而组间的差异则足够小。
...
建议是 1 类，和我们的预期不一致。从图上看，如果切分为 5 类也是很不错的选择，所以将这 422 个站点分为五类看看效果：
...
从刚才三个指标的描述上看，betweenness 和 page rank 越大越好，closeness 我取了倒数，所以越小越好。从中心点上看，第 4 类（有 70 个）和第 5 类（有 27 个）是我们需要重点关注的站点类别。
...
这 97 个地方本身就是地铁网络中相对便捷的节点，如果我们还能够知道这些区域的房产均价和二手房供应量信息，那大家上车的姿势就可以确定了。
...
1. 立水桥、九龙山、管庄这些地方从来都不在视野之内，未来应该多给一些关注；
2. 有些二环外，如东城区的一些地方也不是不能考虑，当然还要具体看楼龄和学区情况；
3. 200 套以上，均价在 75000 以下在图的左上方，说明的是选择空间大，预算相对可控；
4. 可以把具体某个站点的数据单拎出来细看，包括三个核心中心度指标和二手房小区信息。

当然每位看官关注点必然不同，我也就能帮到这里了。未来不论是各位是新上车还是改善需求，有行动也记得给我一些反馈。

获取数据，数据预处理，作图代码量共计 190 行，见附录。全文完毕，收工！
